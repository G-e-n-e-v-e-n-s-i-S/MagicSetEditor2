...
inline static ScriptValueP json_to_mse(boost::json::value& jv, SetP set);

//template <typename K, typename V>
//static void read(IndexMap<K,V>& out, boost::json::object& jv, const char value_name[]) {
//  if (!jv.contains(value_name)) return;
//  else {
//    wxStringInputStream stream = {_("")};
//    Reader reader(stream, nullptr, _(""));
//    boost::json::value value = jv[value_name];
//    if (!value.is_object()) {
//      queue_message(MESSAGE_ERROR, _ERROR_("json to script expected object"));
//      return;
//    }
//    boost::json::object object = value.as_object();
//    for (auto it = object.begin(); it != object.end(); ++it) {
//      V element;
//      reader.setValue(wxString(it->value().as_string().c_str()));
//      reader.handle(element);
//      out.add(it->key(), element);
//    }
//  }
//}
//
//template <typename T>
//static void read(vector<T>& out, boost::json::object& jv, const char value_name[]) {
//  if (!jv.contains(value_name)) return;
//  else {
//    wxStringInputStream stream = {_("")};
//    Reader reader(stream, nullptr, _(""));
//    boost::json::value value = jv[value_name];
//    if (!value.is_array()) {
//      queue_message(MESSAGE_ERROR, _ERROR_("json to script expected array"));
//      return;
//    }
//    boost::json::array array = value.as_array();
//    for (size_t i = 0; i < array.size(); i++) {
//      T element;
//      reader.setValue(wxString(array[i].as_string().c_str()));
//      reader.handle(element);
...
template <typename T>
static void read(T& out, T& value_default, boost::json::object& jv, const char value_name[]) {
  if (!jv.contains(value_name)) out = value_default;
  else {
    wxStringInputStream stream = {_("")};
    Reader reader(stream, nullptr, _(""));
    reader.setValue(wxString(jv[value_name].as_string().c_str()));
...
// templates don't work with enums? are you kidding me with this language?
static void read(PackSelectType& out, PackSelectType value_default, boost::json::object& jv, const char value_name[]) {
  if (!jv.contains(value_name)) out = value_default;
  else {
    wxStringInputStream stream = {_("")};
    Reader reader(stream, nullptr, _(""));
...
//inline static ScriptValueP json_to_mse(boost::json::object& jv, boost::json::string mse_object_type) {
//  if (mse_object_type == "set") {
//    if (!jv.contains("game")) {
//      queue_message(MESSAGE_ERROR, _ERROR_("json to script set without game"));
//      return ScriptValueP();
//    }
//    if (!jv.contains("stylesheet")) {
//      queue_message(MESSAGE_ERROR, _ERROR_("json to script set without stylesheet"));
//      return ScriptValueP();
//    }
...
//  } else if (mse_object_type == "card") {
//
//  } else if (mse_object_type == "keyword") {
//
//  }
//}
...
inline static KeywordP json_to_mse_keyword(boost::json::object& jv) {
  ...
  read(keyword->keyword,  wxString("unnamed keyword"), jv, "keyword");
...
inline static PackTypeP json_to_mse_pack_type(boost::json::object& jv) {
  ...
  read(pack_type->enabled,    Scriptable<bool>(true),               jv, "enabled");
  read(pack_type->selectable, tribool(boost::logic::indeterminate), jv, "selectable");
  read(pack_type->summary,    tribool(boost::logic::indeterminate), jv, "summary");
  read(pack_type->select,     SELECT_AUTO,                          jv, "select");
  return pack_type;
}
inline static PackItemP json_to_mse_pack_item(boost::json::object& jv) {
  PackItemP pack_item = make_intrusive<PackItem>();
  read(pack_item->name,   String("unnamed pack item"), jv, "name");
  read(pack_item->amount, Scriptable<int>(1),          jv, "amount");
  read(pack_item->weight, Scriptable<double>(1.0),     jv, "weight");
  return pack_item;
}
...
inline static ScriptValueP json_to_mse(boost::json::value& jv, SetP set) {
  if (jv == nullptr) return script_nil;
    ...
    if (object.contains("mse_object_type")) {
      ...
      if (mse_object_type == "set") {
        return to_script(json_to_mse_set(object));
      } else if (mse_object_type == "card") {
        return to_script(json_to_mse_card(object, set));
      } else if (mse_object_type == "keyword") {
        return to_script(json_to_mse_keyword(object));
      } else if (mse_object_type == "pack_type") {
        return to_script(json_to_mse_pack_type(object));
      } else if (mse_object_type == "pack_item") {
        return to_script(json_to_mse_pack_item(object));
      }
      queue_message(MESSAGE_ERROR, _ERROR_("json to script cant recognize type"));
      return ScriptValueP();
    }
    ScriptCustomCollectionP result = make_intrusive<ScriptCustomCollection>();
    for (auto it = object.begin(); it != object.end(); ++it) {
...
inline static ScriptValueP json_to_mse(String& string, SetP set) {
  try {
    boost::json::value jv = boost::json::parse(string.ToStdString());
    return json_to_mse(jv, set);
  }
  catch (...) {
    queue_message(MESSAGE_ERROR, string.ToStdString());
    queue_message(MESSAGE_ERROR, _ERROR_("json to script cant parse"));
    return script_nil;
  }
}
inline static ScriptValueP json_to_mse(ScriptValueP& sv, SetP set) {
  try {
    String string = sv->toString();
    return json_to_mse(string, set);
  }
  catch (...) {
    queue_message(MESSAGE_ERROR, _ERROR_("json to script cant convert"));
    return script_nil;
  }
}
...
inline static boost::json::object mse_to_json(CardP& card, SetP set) {
...
inline static boost::json::object mse_to_json(SetP set) {
  ...
  queue_message(MESSAGE_ERROR, json_pretty_print(setv));
  // done
  return setv;
}

inline static boost::json::value mse_to_json(ScriptValueP& sv, SetP set) {
  ...
  if (ScriptObject<SetP>*      s = dynamic_cast<ScriptObject<SetP>*>     (sv.get())) return mse_to_json(s->getValue());
  // primitive types
    ...
    if (custom) {
      ...
      if (concat) {
        ...
        if (a.is_array() && b.is_array()) {
          ...
          queue_message(MESSAGE_ERROR, _ERROR_("script to json cant concat"));
          return boost::json::value(nullptr);
        }
      }
    }
  }
  queue_message(MESSAGE_ERROR, _ERROR_1_("script to json cant convert", sv->typeName()));
  return boost::json::value(nullptr);
}
